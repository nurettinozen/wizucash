...
#if defined(NDEBUG)
# error "Wizucash cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x7f9f6ea5b7436dc5fac4e43d9765da25e5388e295e0fe1d282bec65b37ed79d8");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Wizucash: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Wizucash Signed Message:\n";
...
{
    // Wizucash: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Wizucash: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Wizucash: 5 days
static const int64 nTargetSpacing = 5 * 60; // Wizucash: 5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Wizucash: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Wizucash: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x5411db0dea247464d207f6f7d4011dec15d428ae375184e58cbf0dc123495ed0");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "TR Times 05/Mart/2018 EbaMüslüm Turan, Hasan Gültekin, Nurettin Özen";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("048aa545b2759348dc533b968b9c3fabf77b030405548e556f94c1c6f7bc8974d5da1cef7c61aae967131db816ae2777c1e03022dfb008d205f50d9b0b4f709be2") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1521106843;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084760104;

        if (fTestNet)
        {
            block.nTime    = 1521106829;
            block.nNonce   = 388403888;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xa5a76d0b4c4f0b9c6d39bc56f467ddf59f934347aab73db6288e0ac6ab855b3e"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc2, 0xb5, 0xdb }; // Wizucash: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("WizucashMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("WizucashMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static WizucashMiner(CWallet *pwallet)
{
    printf("WizucashMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("wizucash-miner");
    ...
    try { loop {
        ...
        printf("Running WizucashMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("WizucashMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&WizucashMiner, pwallet));
}
